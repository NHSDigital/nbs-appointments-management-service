...
resource "azurerm_cdn_frontdoor_endpoint" "nbs_mya_endpoint" {
  count                    = var.create_frontdoor ? 1 : 0
  name                     = var.environment == "perf" || var.environment == "int" ? "nbs-mya-${var.environment}" : "nbs-mya"
...
resource "azurerm_cdn_frontdoor_rule" "nbs_mya_origin_group_query_override_rule" {
  ...
  conditions {
    url_path_condition {
      ...
      match_values     = ["/sites", "/availability/query", "/availability/query/days", "/availability/query/hours", "/availability/query/slots"]
