...
public class UserStoreTests
{
    ...
    {
        _sut = new UserStore(_cosmosStore.Object);
    }

    [Fact]
    public async Task UpdateUserRoleAssignments_WhenNoUser_CreatesUser() 
    {
        _cosmosStore.Setup(x => x.GetByIdOrDefaultAsync(It.IsAny<string>())).ReturnsAsync((UserDocument)null);
        ...
        _cosmosStore.Verify(x => x.GetByIdOrDefaultAsync(It.IsAny<string>()), Times.Once);
        _cosmosStore.Setup(x => x.GetDocumentType()).Returns("user");
    ...
    {
        _cosmosStore.Setup(x => x.GetByIdOrDefaultAsync(expected)).ReturnsAsync(new UserDocument { Id = expected, RoleAssignments = new RoleAssignment[] { new() { Role = "test", Scope = "*" } } });
        await _sut.GetUserRoleAssignments(email);
        _cosmosStore.Verify(x => x.GetByIdOrDefaultAsync(expected), Times.Once);
    }
    ...
    {
        _cosmosStore.Setup(x => x.GetByIdOrDefaultAsync(expected)).ReturnsAsync(new UserDocument { Id = expected });
    ...
    public async Task UpdateUserRoleAssignments_WhenAddRoles_UpdatesUser()
    {
        ...
        _cosmosStore.Setup(x => x.GetByIdOrDefaultAsync(It.IsAny<string>())).ReturnsAsync(user);
        _cosmosStore.Setup(x => x.GetDocumentType()).Returns("user");
    ...
    public async Task UpdateUserRegionPermissions_UpdatesUserDocument()
    {
        ...
        _cosmosStore.Verify(x => x.GetByIdOrDefaultAsync("test.user@nhs.net"), Times.Once);
    ...
    {
        _cosmosStore.Setup(x => x.GetByIdOrDefaultAsync(It.IsAny<string>()))
    ...
    {
        var roleAssigments = new List<RoleAssignment> { new() { Role = "system:icb-user", Scope = "icb:ICB1" } };
        var user = new UserDocument()
        {
            Id = "test.user@nhs.net",
        ...
        var result = _sut.GetUpdatedPermissions(
            user.RoleAssignments.Select(MapToRoleAssignment),
            "icb:ICB1",
            [.. roleAssigments.Select(MapToRoleAssignment)],
            IcbRole,
            RegionRole);
    ...
    {
        var roleAssigments = new List<RoleAssignment> { new() { Role = IcbRole, Scope = "icb:ICB1" } };
        var newRoleAssignments = new List<RoleAssignment> { new() { Role = IcbRole, Scope = "icb:ICB2" } };
        var user = new UserDocument()
        {
            Id = "test.user@nhs.net",
            RoleAssignments = [.. roleAssigments]
        ...
        var result = _sut.GetUpdatedPermissions(
                user.RoleAssignments.Select(MapToRoleAssignment),
                "icb:ICB2",
                newRoleAssignments.Select(MapToRoleAssignment),
                IcbRole,
                RegionRole);
    ...
    {
        var newRoleAssignments = new List<RoleAssignment> { new() { Role = IcbRole, Scope = "icb:ICB1" } };
        var user = new UserDocument
        {
            Id = "test.user@nhs.net",
            RoleAssignments = []
    ...
    private static Core.Users.RoleAssignment MapToRoleAssignment(RoleAssignment document)
    {
        return new Core.Users.RoleAssignment
        {
            Role = document.Role,
