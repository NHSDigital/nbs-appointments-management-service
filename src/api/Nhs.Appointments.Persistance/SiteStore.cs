...
public class SiteStore(
    ITypedDocumentCosmosStore<SiteDocument> cosmosStore,
    IMetricsRecorder metricsRecorder
    ) : ISiteStore
{
    public async Task<Site> GetSiteById(string siteId)
    {
        using (metricsRecorder.BeginScope(MetricScopes.Sites.GetById))
        {
    ...
    {
        using (metricsRecorder.BeginScope(MetricScopes.Sites.GetAll))
        {
            return await cosmosStore.RunQueryAsync<Site>(sd => sd.DocumentType == "site"); 
        }
    }

    public async Task<int> GetReferenceNumberGroup(string site)
    {
        using (metricsRecorder.BeginScope(MetricScopes.Sites.GetReferenceNumberGroup))
        {
    ...
    {
        using (metricsRecorder.BeginScope(MetricScopes.Sites.UpdateSiteReferenceDetails))
        {
    ...
    {
        using (metricsRecorder.BeginScope(MetricScopes.Sites.AssignPrefix))
        {
    ...
    {
        using (metricsRecorder.BeginScope(MetricScopes.Sites.UpdateAccessibilityInfo))
        {
            var originalDocument = await GetOrDefault(siteId);
            if (originalDocument == null ||
                !ValidateUpdateToSiteAllowed(originalDocument))
    ...
    {
        using (metricsRecorder.BeginScope(MetricScopes.Sites.UpdateInformationForCitizens))
        {
            var originalDocument = await GetOrDefault(siteId);
            if (originalDocument == null ||
                !ValidateUpdateToSiteAllowed(originalDocument))
        ...
        decimal? longitude, decimal? latitude, string type = null)
    {
        ...
        using (metricsRecorder.BeginScope(MetricScopes.Sites.UpdateSiteDetails))
        {
            var originalDocument = await GetOrDefault(siteId);
            if (originalDocument == null ||
                !ValidateUpdateToSiteAllowed(originalDocument))
    ...
    {
        using (metricsRecorder.BeginScope(MetricScopes.Sites.Save))
        {
            var originalDocument = await GetOrDefault(siteId);
    ...
    {
        using (metricsRecorder.BeginScope(MetricScopes.Sites.GetSitesInRegion))
        {
    ...
    {
        using (metricsRecorder.BeginScope(MetricScopes.Sites.UpdateSiteStatus))
        {
            var originalDocument = await GetOrDefault(siteId);
            if (originalDocument is null ||
                !ValidateUpdateToSiteAllowed(originalDocument))
    ...
    {
        using (metricsRecorder.BeginScope(MetricScopes.Sites.GetSitesInIcb))
        {
            return await cosmosStore.RunQueryAsync<Site>(s => s.DocumentType == "site" && s.IntegratedCareBoard == icb); 
        }
    }

    public async Task<OperationResult> ToggleSiteSoftDeletionAsync(string siteId)
    {
        using (metricsRecorder.BeginScope(MetricScopes.Sites.ToggleSiteSoftDeletion))
        {
            var originalDocument = await GetOrDefault(siteId);
            if (originalDocument is null)
            {
