...
public class QueryAvailabilityFunctionTests
{
    ...
    public async Task RunAsync_ReturnsSuccessResponse_WhenMultipleSitesAreQueried()
    {
        ...
        _hasConsecutiveCapacityFilter.Setup(x => x.SessionHasConsecutiveSessions(It.IsAny<HashSet<SessionInstance>>(), It.IsAny<int>())).ReturnsAsync(slots.ToHashSet);
        ...
        if (await _sut.RunAsync(httpRequest) is ContentResult result)
        {
            ...
            _hasConsecutiveCapacityFilter.Verify(
                x => x.SessionHasConsecutiveSessions(It.IsAny<HashSet<SessionInstance>>(), It.IsAny<int>()),
                Times.Never);
    ...
    public async Task RunAsync_RunsJointBookings_WhenJointBookingsEnabled()
    {
        ...
        _hasConsecutiveCapacityFilter.Verify(x => x.SessionHasConsecutiveSessions(It.IsAny<HashSet<SessionInstance>>(), It.IsAny<int>()), Times.Once);
    }
    ...
    public async Task RunAsync_CallsGrouperWithCorrectSlots_ForEachDayInRequest()
    {
        ...
        _availabilityGrouper.Setup(x => x.GroupAvailability(It.IsAny<IEnumerable<SessionInstance>>()))
            .Returns(responseBlocks);
        _featureToggleHelper.Setup(x => x.IsFeatureEnabled(It.Is<string>(p => p == Flags.JointBookings))).ReturnsAsync(false);
        _hasConsecutiveCapacityFilter.Setup(x => x.SessionHasConsecutiveSessions(It.IsAny<HashSet<SessionInstance>>(), It.IsAny<int>())).ReturnsAsync(blocks.ToHashSet);
        _siteService.Setup(x => x.GetAllSites(It.IsAny<bool>(), It.IsAny<bool>()))
            .ReturnsAsync(new List<Site>
            {
                new(
        ...
        _availabilityGrouper.Verify(x => x.GroupAvailability(It.IsAny<IEnumerable<SessionInstance>>()),
            Times.Exactly(3));
        _hasConsecutiveCapacityFilter.Verify(x => x.SessionHasConsecutiveSessions(It.IsAny<HashSet<SessionInstance>>(), It.IsAny<int>()), Times.Never);
    }
    ...
    public async Task RunAsync_ReturnsEmptyArray_WhenAllRequestedSites_HaveBeenSoftDeleted()
    {
        ...
        _availabilityGrouper.Verify(x => x.GroupAvailability(It.IsAny<IEnumerable<SessionInstance>>()),
            Times.Never);
        _hasConsecutiveCapacityFilter.Verify(x => x.SessionHasConsecutiveSessions(It.IsAny<HashSet<SessionInstance>>(), It.IsAny<int>()),
            Times.Never);
    }
