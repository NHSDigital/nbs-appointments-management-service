...
public class SetSiteDetailsFunction(ISiteService siteService, IValidator<SetSiteDetailsRequest> validator, IUserContextProvider userContextProvider, ILogger<SetSiteDetailsRequest> logger, IMetricsRecorder metricsRecorder) 
    : BaseApiFunction<SetSiteDetailsRequest, EmptyResponse>(validator, userContextProvider, logger, metricsRecorder)
{
    ...
    [OpenApiResponseWithBody(statusCode:HttpStatusCode.NotFound, "application/json", typeof(ApiResult<object>), Description = "Site details not found")]
    ...
    public override Task<IActionResult> RunAsync(
        [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "sites/{site}/details")] HttpRequest req)
    {
    ...
    {
        var result = await siteService.UpdateSiteDetailsAsync(request.Site, request.Name);
        return result.Success ? Success(new EmptyResponse()) : Failed(HttpStatusCode.NotFound, result.Message);
    }

    protected override async Task<(IReadOnlyCollection<ErrorMessageResponseItem> errors, SetSiteDetailsRequest request)> ReadRequestAsync(HttpRequest req)
    {
        ...
        return (errors, new SetSiteDetailsRequest(site, details.Name));
    }
}
