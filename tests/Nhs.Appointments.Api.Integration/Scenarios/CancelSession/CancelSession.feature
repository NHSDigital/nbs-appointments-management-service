Feature: Cancel a session
  ...
  Scenario: Greedy allocation alphabetical - suboptimal - Appointment status for provisional is recalculated and deleted after availability is cancelled
    ...
    When I cancel the following session at the default site
      | Date     | From  | Until | Services     | Slot Length | Capacity |
      | Tomorrow | 09:00 | 17:00 | RSV, FLU     | 5           | 1        |
    Then the booking at the default site with reference '37492-16293' has status 'Provisional'
    And the booking at the default site with reference '37492-16293' has availability status 'Supported'
    And the booking at the default site with reference '89999-44622' should be deleted
    Then the booking at the default site with reference '67834-56421' has status 'Provisional'
    And the booking at the default site with reference '67834-56421' has availability status 'Supported'
