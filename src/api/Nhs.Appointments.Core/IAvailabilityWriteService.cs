...
public interface IAvailabilityWriteService
{
    ...
    Task ApplySingleDateSessionAsync(DateOnly date, string site, Session[] sessions, ApplyAvailabilityMode mode,
        string user, Session sessionToEdit = null, bool CancelOrphaned = false);
    ...
    Task<(bool updateSuccessful, string message)> EditOrCancelSessionAsync(string site, DateOnly from, DateOnly until, Session? sessionMatcher, Session? sessionReplacement, bool isWildcard, bool cancelUnsupportedBookings);
}
