locals {
  ...
  resource_group_name = var.environment == "perf" ? "${var.application}perf-rg-stag-${var.loc}" : var.environment == "dev" ? "${var.application}-${var.environment}-${var.loc}" : "${var.application}-rg-${var.environment}-${var.loc}"
}
