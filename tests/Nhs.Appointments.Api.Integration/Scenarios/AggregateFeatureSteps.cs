...
{
    [Then(@"an aggregation exists for site '(.*)', date '(.*)', '(.*)' cancelled bookings, and maximumCapacity '(.*)', and with service details")]
    [And(@"an aggregation exists for site '(.*)', date '(.*)', '(.*)' cancelled bookings, and maximumCapacity '(.*)', and with service details")]
    public async Task AssertAggregationDocumentFor(string site, string day, int expectedCancelled, int expectedMaximumCapacity, DataTable servicesTable)
    {
