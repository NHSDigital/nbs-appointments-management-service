...
export const UsersList = async ({ site }: Props ) => {
    ...
    const getRoleName = (role: string) => data.find(r => r.id === role)?.displayName;

    return (
        <ul className="divide-y divide-gray-200 dark:divide-gray-700" style={{marginLeft: "12px"}}>
        {users.map(usr => (
            <li key={usr.id} className="pb-3 sm:pb-4" style={{paddingTop: "8px"}}>
            <div className="flex items-center space-x-4 rtl:space-x-reverse">
               ...
               <ul className="flex flex-1 flex-wrap items-center justify-center text-gray-900 dark:text-white">
                {usr.roleAssignments.map(ra => (
                    <li key={ra.role} style={{borderRight: "1px solid black", paddingRight: "6px", paddingLeft: "6px"}}>
                        {getRoleName(ra.role)}  
                    </li>
                ))}
                <Authorized requiredPermission="users:manage" site={site}>
                    <li>                    
