...
public abstract class UpdateSiteReferenceDetailsFeatureSteps : SiteManagementBaseFeatureSteps
{
    ...
    [When("I update reference details for site '(.+)' for api user '(.+)'")]
    public async Task UpdateSiteReferenceDetails(string siteDesignation, string apiUser, DataTable dataTable)
    {
        ...
        var payload = new SetSiteReferenceDetailsRequest(siteId, odsCode, icb, region);
        Response = await customHttpClient.PostAsJsonAsync($"http://localhost:7071/api/sites/{siteId}/reference-details", payload);
    }
}
