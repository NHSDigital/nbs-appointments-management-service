...
public class AggregatorSink(ILogger<AggregatorSink> logger, ISiteSummaryAggregator siteSummaryAggregator) : ISink<AggregateSiteSummaryEvent>
{
    public async Task Consume(string source, AggregateSiteSummaryEvent item)
    {
        ...
        await siteSummaryAggregator.AggregateForSite(item.Site, item.Date, item.Date);
    }
}
